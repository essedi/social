...
class TestMailGatewayWhatsApp(MailGatewayTestCase):
    ...
    def test_receive_message_02(self):
        ...
        self.assertFalse(message.parent_id)
        # The channel store data exposes the gateway partner and the gateway
        channel = self.env["discuss.channel"].search(
            [("gateway_id", "=", self.gateway.id)]
        )
        channel_data = Store().add(channel).get_result()["discuss.channel"][0]
        self.assertEqual(channel_data["channel_type"], "gateway")
        self.assertEqual(channel_data["gateway_partner_id"], partner.id)
        self.assertEqual(channel_data["gateway_id"]["gateway_type"], "whatsapp")
