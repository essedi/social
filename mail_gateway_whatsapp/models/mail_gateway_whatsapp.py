            ...
            if related_message_id:
                ...
                if related_message and related_message.gateway_message_id:
                    ...
                    Store(bus_channel=new_message._bus_channel()).add(
                        new_message, ["gateway_thread_data"]
                    ).bus_send()
    ...
    def _get_author(self, gateway, update):
        ...
        if author_id:
            ...
            if partner:
                ...
                # A partner can only have one configuration per gateway: if it
                # already exists with another token (e.g. another number), keep
                # it and still attribute the message to the partner.
                if GatewayChannel.search(
                    [("partner_id", "=", partner.id), ("gateway_id", "=", gateway.id)],
                    limit=1,
                ):
                    _logger.info(
                        "Partner %s already has a configuration for gateway %s, "
                        "not linking token %s",
                        partner.id,
                        gateway.id,
                        author_id,
